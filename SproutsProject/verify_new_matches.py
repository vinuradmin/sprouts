#!/usr/bin/env python3
"""
Verify if all NEW Hungarian algorithm matches were in original algorithm
"""

import csv
import re

def verify_new_matches():
    """Verify if current matches were in original algorithm"""
    print("=== VERIFYING NEW HUNGARIAN ALGORITHM MATCHES ===")
    ...
    # NEW Hungarian algorithm matches after fix
    new_matches = [
        ("Zhijian Liu", "alaMar Dominican Kitchen"),
    ...
    print(f"Total new matches: {len(new_matches)}")
    print(f"Verified in original: {verified_count}")
    print(f"Not found in original: {not_found_count}")
    print(f"Verification rate: {verified_count/len(new_matches)*100:.1f}%")
    ...
    if not_found_count > 0:
        ...
        print(f"\nAll new matches were available in original algorithm")
