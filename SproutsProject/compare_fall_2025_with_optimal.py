#!/usr/bin/env python3
"""
Compare Fall 2025 actual matches with optimal algorithm
"""
...
def get_fall_2025_interns_and_matches():
    """Get Fall 2025 interns and their actual matches"""
    print("=== GETTING FALL 2025 INTERNS AND MATCHES ===")
    ...
    try:
        ...
        # Find Fall 2025 interns (starting from row 337)
        fall_2025_interns = []
        ...
        for idx in range(338, 367):  # Fall 2025 section
            if idx < len(active_df):
                ...
                if intern_name and intern_name.lower() != 'nan' and 'latitude' not in intern_name.lower():
                    # Get restaurant assignment from column 9 (index 9)
                    restaurant = str(row.iloc[9]).strip() if pd.notna(row.iloc[9]) else ''
                    ...
                    fall_2025_interns.append({
                        ...
                        'full_info': [str(val) if pd.notna(val) else '' for val in row]
                    })
        
        print(f"Found {len(fall_2025_interns)} Fall 2025 interns:")
        for intern in fall_2025_interns:
            print(f"  - {intern['name']} -> {intern['restaurant']}")
        
        return fall_2025_interns
        
    except Exception as e:
        print(f"Error getting Fall 2025 interns: {e}")
        return []

def compare_with_optimal_algorithm(fall_2025_interns):
    """Compare Fall 2025 matches with optimal algorithm"""
    ...
    try:
        ...
        # Compare Fall 2025 interns
        comparison_results = []
        
        for fall_intern in fall_2025_interns:
            fall_name = fall_intern['name']
            actual_restaurant = fall_intern['restaurant']
            ...
            for assignment in optimal_assignments:
                ...
                if (fall_name.lower() in opt_name.lower() or 
                    opt_name.lower() in fall_name.lower() or
                    fall_name.split()[0].lower() in opt_name.lower() or
                    opt_name.split()[0].lower() in fall_name.lower()):
            ...
            if optimal_match:
                ...
                print(f"\n{fall_name}: No optimal assignment found")
        ...
        print(f"\n=== COMPARISON SUMMARY ===")
        print(f"Total Fall 2025 interns: {len(fall_2025_interns)}")
        ...
        if different_matches:
            ...
            for match in different_matches:
                ...
                if isinstance(match['optimal_commute'], int) and match['optimal_commute'] < 60:
            ...
            for idx in range(338, 367):
                if idx < len(active_df):
                    ...
                    if target_intern.lower() in intern_name.lower():
                        actual_restaurant = str(active_df.iloc[idx, 9]).strip()
...
if __name__ == "__main__":
    # Get Fall 2025 interns and matches
    fall_2025_interns = get_fall_2025_interns_and_matches()
    
    if fall_2025_interns:
        # Compare with optimal algorithm
        comparison_results = compare_with_optimal_algorithm(fall_2025_interns)
