#!/usr/bin/env python3
"""
Show availability for Ollie O'Malley and Sorrel Restaurant from CSV files
"""
...
def show_ollie_sorrel_csv():
    """Show availability for Ollie and Sorrel from CSV"""
    print("=== OLLIE O'MALLEY & SORREL RESTAURANT CSV AVAILABILITY ===")
    
    # Get Ollie's availability from intern CSV
    print("\n1. OLLIE O'MALLEY AVAILABILITY (from intern_avail_fall.csv):")
    try:
    ...
    # Get Sorrel Restaurant availability from chef CSV
    print("\n2. SORREL RESTAURANT AVAILABILITY (from chef_avail_fall.csv):")
    try:
    ...
    # Calculate overlap from CSV data
    print("\n3. CSV OVERLAP ANALYSIS:")
    try:
        # Ollie availability
        ollie_avail = {}
        with open('../intern_avail_fall.csv', 'r', encoding='utf-8') as file:
        ...
        with open('../chef_avail_fall.csv', 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                if 'Sorrel' in restaurant_name:
                    ...
                    for day in days:
                        time_str = row.get(day, '').strip()
                        sorrel_avail[day] = parse_time_slots(time_str)
                    break
        ...
        for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']:
            ...
            sorrel_slots = sorrel_avail.get(day, [])
            
            if ollie_slots and sorrel_slots:
                print(f"  {day} overlap:")
                for o_start, o_end in ollie_slots:
                    for s_start, s_end in sorrel_slots:
        ...
        # Show the issue with parsing
        print(f"\n4. PARSING ISSUE ANALYSIS:")
        print("  Sorrel's availability is stored as seven 1-hour slots:")
        print("  Wednesday: 10AM-11AM, 11AM-12PM, 12PM-1PM, 1PM-2PM, 2PM-3PM, 3PM-4PM, 4PM-5PM")
        print("  This should be merged into one 7-hour slot: 10AM-5PM")
        print("  But the Hungarian algorithm sees seven separate 1-hour slots")
        print("  Since each slot is < 4 hours, it gets rejected by the 4-hour minimum rule")
        
    except Exception as e:
        print(f"Error calculating overlap: {e}")

if __name__ == "__main__":
    show_ollie_sorrel_csv()
